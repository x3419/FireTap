...
BOOL IsNetworkProc(DWORD dwPid)
{
	...
	// to debug we wanted to print all of the proc args and their len

	//if(len > -1){
	//	char *thing = (char *)MSVCRT$malloc(len);
	//	size_t ret = MSVCRT$wcstombs(thing, lpwBufferLocal, len);
	//	BeaconPrintf(CALLBACK_OUTPUT, thing);
	//	BeaconPrintf(CALLBACK_OUTPUT, "%d", len);
	//}
...
DWORD GetChromeNetworkProc()
{
    ...
    do
    {
        ...
        if (MSVCRT$wcscmp(lpProcInfo->ProcessName.Buffer, L"firefox.exe") == 0)
        {
